...
GooFlow.prototype = {
	...
	initGroup : function(width, height) {
		...
		this.$group.on("mousedown", {
			...
			if (cursor != "move") {
				This.$ghost.css({
					...
					left : This.$areaData[id].left + t.left - This.$workArea[0].parentNode.scrollLeft + "px",
					cursor : cursor
				});
		...
		this.$group.mouseup({
			...
			switch ($(e.target).attr("class")) {
			case "rs_close":
				This.delArea(e.target.parentNode.parentNode.id);return false; //删除该分组区域
			case "bg":
				return;
			}
			switch (e.target.tagName) {
	...
	initExpendFunc : function() {
		...
		this.$workArea.children(".Gooflow_extend_right").on("click", {
			inthis : this
		}, function(e) {
			var This = e.data.inthis;
	...
	addNode : function(id, json) {
		...
		if (json.type.indexOf(" round") < 0) {
			if (!json.width || json.width < 104)
				json.width = 104;
			if (!json.height || json.height < 26)
				json.height = 26;
			...
			document.onmousemove = function(e) {
				...
				if (isMove && This.$ghost.css("display") == "none") {
					This.$ghost.css({
						...
						left : This.$nodeData[id].left + t.left - This.$workArea[0].parentNode.scrollLeft + "px",
						cursor : "move"
					});
				}
		...
		if (type == "node") { //如果是结点
			...
			if (GooFlow.prototype.useSVG != "") {
				...
				if (this.$lineData[id].type != "lr") {
					...
					if (Min > this.$lineData[id].M)
						Min = this.$lineData[id].M;
	...
	drawLine : function(id, sp, ep, mark, dash) {
		...
		if (GooFlow.prototype.useSVG != "") {
			...
			if (mark)
				line.strokeColor = GooFlow.prototype.color.mark || "#ff8800";
			else
	...
	drawPoly : function(id, sp, m1, m2, ep, mark) {
		...
		if (GooFlow.prototype.useSVG != "") {
			...
			poly.style.cursor = "pointer";
			if (mark)
				poly.strokeColor = GooFlow.prototype.color.mark || "#ff8800";
			else
