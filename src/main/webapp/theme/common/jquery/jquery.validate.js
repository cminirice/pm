...
(function(c) {
	c.extend(c.fn, {
		...
		rules : function(a, b) {
			...
			if (a) {
				...
				switch (a) {
				case "add":
					c.extend(g, c.validator.normalizeRule(b));f[d.name] = g;
					if (b.messages)
	...
	c.validator = function(a,
		...
		this.currentForm = b;this.init()
	};
	c.validator.format = function(a, b) {
		if (arguments.length == 1) return function() {
				var d = c.makeArray(arguments);
				d.unshift(a);return c.validator.format.apply(this, d)
			};
		...
		prototype : {
			init : function() {
				function a(e) {
					...
					e = "on" + e.type.replace(/^validate/, "");f.settings[e] && f.settings[e].call(f, this[0])
				}
			...
			form : function() {
				this.checkForm();c.extend(this.submitted, this.errorMap);
				this.invalid = c.extend({}, this.errorMap);this.valid() || c(this.currentForm).triggerHandler("invalid-form", [ this ]);this.showErrors();return this.valid()
			},
			checkForm : function() {
				this.prepareForm();
			...
			element : function(a) {
				this.lastElement = a = this.clean(a);this.prepareElement(a);
			...
			resetForm : function() {
				...
				this.submitted = {};this.prepareForm();this.hideErrors();this.elements().removeClass(this.settings.errorClass)
			},
		...
		normalizeRules : function(a, b) {
			c.each(a, function(d, e) {
				if (e === false)
					...
					if (f)
						a[d] = e.param !== undefined ? e.param : true;else
						delete a[d]
				}
			});c.each(a, function(d, e) {
				a[d] = c.isFunction(e) ?
		...
		addMethod : function(a, b, d) {
			...
			c.validator.messages[a] = d != undefined ? d : c.validator.messages[a];b.length < 3 && c.validator.addClassRules(a, c.validator.normalizeRule(a))
		},
		methods : {
			...
			remote : function(a, b, d) {
				...
				this.startRequest(b);
				var g = {};
				g[b.name] = a;c.ajax(c.extend(true, {
					url : d,
					mode : "abort",
					...
					success : function(h) {
						...
						if (j) {
							...
							f.prepareElement(b);
							f.formSubmitted = i;f.successList.push(b);f.showErrors()
						} else {
							i = {};
							h = h || f.defaultMessage(b, "remote");
							i[b.name] = e.message = c.isFunction(h) ? h(a) : h;f.showErrors(i)
						}
...
(function(c) {
	!jQuery.event.special.focusin && !jQuery.event.special.focusout && document.addEventListener && c.each({
		...
		function d(e) {
			e = c.event.fix(e);
			e.type = b;return c.event.handle.call(this, e)
		}
		c.event.special[b] = {
			setup : function() {
