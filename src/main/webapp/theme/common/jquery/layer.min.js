...
;!function(a, b) {
	...
	h.pt = h.prototype, h.pt.config = {
		...
		case "left-top":
			...
			break;case "right-top":
			g.css({
				left : k.cr,
				top : k.ct
			}).animate(k.t, b);
			break;case "right-bottom":
			g.css({
				left : k.cr,
				top : j
			}).animate(c ? k.t : k.b, b);
		...
		if (b[0]) {
			if (d == f.type[1])
				if (b.find(".xuboxPageHtml")[0]) b[0].innerHTML = "", b.remove();else {
			...
			e.remove(), layer.ie6 && f.reselect(), f.rescollbar(a), "function" == typeof f.config.end[a] && f.config.end[a](),
		...
		c = jQuery, d = c(a), g.html = c("html"), layer.use("skin/layer.css"), c.layer = function(a) {
			var b = new h(a);
			return b.index
