...
<head>
    ...
    <title>新增角色 - Powered By GUTTV</title>
...
<body class="input">
    <div class="body">
        <h2 class="title-role" style="color: #5686af">新增角色</h2>
        <form id="roleForm" action="" method="post" style="margin-top: 50px;">
            <label class="label-role margin-letf" for="roleId">角色编号:</label>
            <input class="input-role" type="number" id="roleId" name="id" autocomplete="off"/></br>
            <label class="label-role margin-letf" for="roleName">角色名称:</label>
            <input class="input-role" type="text" id="roleName" name="name" autocomplete="off"></br>
            <label class="label-role margin-letf" for="roleDescription">角色描述:</label>
            <input class="input-role" type="text" id="roleDescription" name="description" autocomplete="off"></br>
            ...
            <c:forEach items="${authList}" var="auth">
                ...
                <input type="checkbox" name="auths" value="${auth.id}" style="margin-left: 80px;"
                       class="input-role"
                       id="<c:if test="${auth.parentAuthId != null and auth.parentAuthId != 0}">${auth.parentAuthId}</c:if><c:if test="${auth.parentAuthId == null or auth.parentAuthId == 0}">0000</c:if>${auth.id}"/>
            ...
            <div>
                <input type="button" id="addRoleButton" class="button-role" value="保存"/>
            </div>
        </form>
    </div>
</body>
